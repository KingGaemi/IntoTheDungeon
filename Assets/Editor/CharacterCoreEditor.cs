...
public class CharacterCoreEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (!_target.IsValid)
        {
            EditorGUILayout.HelpBox("Waiting for initialization...", MessageType.Warning);
            return;
        }

        ref var status = ref _target.GetStatus();
    ...
    private void DrawTestSection()
    {
        ...
        {
            _target.TestDamage(_testDamage);
        }
        EditorGUILayout.EndHorizontal();
        ...
        {
            _target.TestAttackSpeed(_testSpeedChange);
        }
        if (GUILayout.Button("Movement Speed"))
        {
            _target.TestMovementSpeed(_testSpeedChange);
        }
        EditorGUILayout.EndHorizontal();
        ...
        {
            _target.TestDamage(9999);
        }
        if (GUILayout.Button("Full Heal"))
        {
            var status = _target.GetStatusComponent();

            _target.TestHeal(status.MaxHp);
